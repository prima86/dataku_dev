var input_var = ['total_bayi_lahir','total_bblr','total_balita','total_balita_gizi_buruk'];
          ...
          function show_data()
          {
            $.each(query, function(index, value){
              $.each(input_var, function(index_input, value_input){
                $('tr[kecamatan-id="'+value.id_kecamatan+'"]').find('td[data-control="'+value_input.data+'"]').text(empty_if_false(value[value_input.input]));
                  });
            })
          }
  ...
  $('tr[data-control="row"]').each(function() {
    ...
    $.each(input_var,function(index_input, value_input) {
      ...
      if(value > 0) {
          ...
          sum_row += parseFloat(value);
      }
    });
    tr.find('td[data-control="data-total-row"]').text(sum_row);
  });

  var sum_total = 0
  $.each(input_var,function(index_input, value_input) {
    var sum_value_input_input = sum[value_input.input];
    $('td#all_'+value_input.input).text(sum_value_input_input);
    sum_total += sum[value_input.input];
  });
  $('td#all_total_row').text(sum_total);
}
...
$('#btn-save').click(function(event) {
  ...
  $.ajax({
      ...
      error:function(){
        notify_alert("Data gagal disimpan...",'warning')
        get_data_rows();
        // default_button();
      }
  });
      ...
      success:function(query)
      {
        ...
        function show_form()
        {
          ...
          $('tr[data-control="row"]').each(function(){
            ...
            if(query.length > 0) {
              //inputbox hidden untuk id_kecamatan
              var input_hidden_id='<input type="hidden" data-control="data-template" name="data_input['+data_index+'][id_kecamatan]" value="'+record.id_kecamatan+'">';
              tr.find('td[data-control="data-list"]').append(input_hidden_id);

              //inputbox untuk edit kolom number
              $.each(input_var, function(index, el){            
              ...
              //inputbox hidden untuk id_kecamatan
              var input_hidden_id_template='<input type="hidden" data-control="id-template" name="data_input['+data_index+'][id_kecamatan]" value="'+list_template[data_index].id_kecamatan+'" class="input-data-table form-control text-center w-100">';
...
$('#btn-set-publish').click(function(event){
  ...
  $('#btn-status').attr('class', 'btn text-white dropdown-toggle btn-success');
  $('#btn-status').html('');
  $('#btn-status').append('<i class="fas fa-info"></i>  Published');
})